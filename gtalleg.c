...
BOOL HB_GT_FUNC(gt_SetMode( USHORT usRows, USHORT usCols ))
{
   ...
   {
      if ((bits==ubits) && (unum_col==num_col) && (unum_lin==num_lin)){
         iRet=0;
      }else{
         set_color_depth(bits);
         iRet = al_set_gfx_mode( GFX_AUTODETECT_FULLSCREEN, num_col, num_lin, 0, 0 );
      ...
      if ( iRet != 0 )  // Doh!
      {
         ...
         {
            printf("gtAlleg FATAL: could not switch to graphic mode. Erro: %d\n",iRet);
            allegro_message("\nErro: `%s'\n", allegro_error);
            exit(1);
         }
      }
      ...
      } else
      {
         ...
         if (bf){
            al_clear_to_color( bf, s_pClr[s_pbyScrBuffer[1] >> 4] );
         }else{
...
HB_FUNC(AL_SET_RGB){
   ...
   g=hb_parni(3);
   b=hb_parni(4);
   s_pClr[indice] = al_make_color(r,g,b);  // bright white  
}
...
//            HB_GT_FUNC(gt_SetMode(_GetScreenHeight(), _GetScreenWidth()));
      ...
      case GTI_SCREENDEPTH:
         if (bf){
           iOldValue = ( s_pbyScrBuffer == NULL ? al_desktop_color_depth() : al_bitmap_color_depth( bf ) );
         }else{
...
int HB_GT_FUNC( gt_gfxPrimitive( int iType, int iTop, int iLeft, int iBottom, int iRight, int iColor ) )
{
   ...
   if (bf){
      dst = s_usDispCount == 0 ? al_screen : bf;
   }else{
...
int iBottom, iRight;
   ...
   if (bf){
      ssfDrawText( bf, ssfDefaultFont, cBuf, iLeft, iTop, iColor );
   }else{
